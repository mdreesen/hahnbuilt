...
import { hasInjectionContext, inject, isRef, toValue, getCurrentScope, ref, watchEffect, getCurrentInstance, onBeforeUnmount, onDeactivated, onActivated, shallowReactive, reactive, effectScope, toRef, createApp, provide, onErrorCaptured, onServerPrefetch, unref, createVNode, resolveDynamicComponent, defineAsyncComponent, mergeProps, shallowRef, isReadonly, computed, defineComponent, createElementBlock, cloneVNode, h, useSSRContext, isShallow, isReactive, toRaw, nextTick, useModel, withCtx, renderSlot, openBlock, createBlock, createTextVNode, toDisplayString, createCommentVNode, mergeModels, watch, Fragment, renderList, useId, isVNode, Comment, useAttrs, useTemplateRef, camelize, toRefs, useSlots, withModifiers, normalizeProps, guardReactiveProps, normalizeStyle, Suspense, resolveComponent, Teleport, toHandlerKey, markRaw, onScopeDispose, queuePostFlushCb } from 'vue';
import { FlatMetaPlugin } from 'unhead/plugins';
import { walkResolver, hasOwn } from 'unhead/utils';
import { c as createError, l as hasProtocol, j as joinURL, w as withQuery, s as sanitizeStatusCode, m as parseURL, e as encodePath, n as decodePath, o as isScriptProtocol, $ as $fetch, q as baseURL, t as defu, v as serialize, x as klona, y as defuFn, z as hash, A as isEqual, B as parseQuery, C as withLeadingSlash, D as withTrailingSlash, E as withoutTrailingSlash, F as encodeParam } from '../nitro/nitro.mjs';
import { START_LOCATION, createMemoryHistory, createRouter, useRoute as useRoute$1, RouterView } from 'vue-router';
import { isPlainObject } from '@vue/shared';
import { _api, addAPIProvider, setCustomIconsLoader, Icon, getIcon, loadIcon, addIcon } from '@iconify/vue';
...
var _gsap = {
  ...
  getProperty: function getProperty(target, property, unit, uncache) {
    _isString$2(target) && (target = toArray$1(target)[0]); //in case selector text or an array is passed in
...
Tween.version = Timeline.version = gsap$4.version = "3.15.0";
_coreReady = 1;
_easeMap.Power0;
    _easeMap.Power1;
    _easeMap.Power2;
...
var gsap$3,
  ...
  ScrollTrigger$2 || _setScrollTrigger(); // Observer might be initted BEFORE ScrollTrigger, so don't put this with the initting code. ScrollTrigger will call Observer.register() when it inits.
...
var virtual_nuxt_node_modules_2F_cache_2Fnuxt_2F_nuxt_2Froutes_default = [
	...
	},
	{
		name: "projects-setup",
		path: "/projects/setup",
		component: () => import('../build/setup-PQsZUibO.mjs')
	},
	{
		name: "about",
		path: "/about",
	...
	},
	{
		name: "contact",
		path: "/contact",
		component: () => import('../build/contact-0m2-f-0H.mjs')
	},
	{
		name: "projects",
		path: "/projects",
		component: () => import('../build/projects-BVohy8vB.mjs')
	},
	{
		name: "index",
		path: "/",
...
//#endregion
//#region node_modules/v-gsap-nuxt/dist/runtime/utils/entrance-presets.js
var entrancePresets = [
	{
		name: "slide-left",
...
//#endregion
//#region node_modules/v-gsap-nuxt/dist/runtime/plugin.js
gsapWithCSS.registerPlugin(ScrollTrigger, ScrollToPlugin, Draggable, TextPlugin, SplitText);
var globalTimelines = {};
var observer;
var intersectionObserver;
var vGsapDirective = (appType, configOptions, gsapContext, resizeListener) => ({
	getSSRProps: (binding) => {
	...
	async beforeMount(el, binding, vnode) {
		...
		if (binding.modifiers.timeline) {
			...
			await nextTick();
			globalTimelines[gsapId] = prepareTimeline(el, binding, configOptions, true);
	...
	async mounted(el, binding) {
		...
		if (binding.modifiers.timeline) {
			...
			if (binding.modifiers.splitText) await nextTick();
			const breakpoint = configOptions?.breakpoint || 768;
			if (binding.modifiers.desktop) mm.add(`(min-width: ${breakpoint}px)`, () => {
				timeline = prepareTimeline(el, binding, configOptions);
			});
			else if (binding.modifiers.mobile) mm.add(`(max-width: ${breakpoint}px)`, () => {
				timeline = prepareTimeline(el, binding, configOptions);
			});
			else timeline = prepareTimeline(el, binding, configOptions);
			if (binding.modifiers.add) {
				nextTick(() => {
					...
					if (!parentTimelineElement?.dataset?.gsapId) return;
...
function prepareSplitText(el, binding) {
	if (binding.modifiers.splitText) {
		let splitType = "chars";
		if (binding.modifiers.lines) splitType = "lines";
		else if (binding.modifiers.words) splitType = "words";
		const splitOptions = {
			type: splitType,
			...binding.value?.splitText || {}
		};
		if (binding.modifiers.mask) {
			if (binding.modifiers.lines) splitOptions.mask = "lines";
			else if (binding.modifiers.words) splitOptions.mask = "words";
			else splitOptions.mask = "chars";
		}
		const onSplitCb = splitOptions.onSplit;
		splitOptions.waitForFonts;
		const doSplit = () => {
			if (el._splitText && typeof el._splitText.revert === "function") try {
				el._splitText.revert();
			} catch (e) {}
			...
			if (binding.modifiers.mask) {
				...
				const containers = [];
				if (splitOptions.mask === "lines" && instance.lines) containers.push(...Array.from(instance.lines).map((line) => line.parentElement).filter(Boolean));
				else if (splitOptions.mask === "words" && instance.words) containers.push(...Array.from(instance.words).map((word) => word.parentElement).filter(Boolean));
				else if (splitOptions.mask === "chars" && instance.chars) containers.push(...Array.from(instance.chars).map((char) => char.parentElement).filter(Boolean));
				containers.forEach((container) => {
					if (container && container.style) {
						container.style.paddingBottom = maskPadding;
						if (!container.style.lineHeight || container.style.lineHeight === "normal") container.style.lineHeight = "normal";
					}
				});
			}
			if (typeof onSplitCb === "function") try {
				onSplitCb({
					el,
					split: instance
				});
			...
			try {
				el.dispatchEvent(new CustomEvent("vgsap:split", { detail: {
					el,
					split: instance
				} }));
			} catch (e) {}
			try {
				const gsapId = el._gsapId || el.dataset.gsapId;
				if (gsapId) ScrollTrigger.getById?.(gsapId)?.refresh?.();
			} catch (e) {}
			return instance;
		};
		return doSplit();
...
function prepareTimeline(el, binding, configOptions, skipSplitText = false) {
	...
	if (binding.modifiers.splitText && !el._splitText && !skipSplitText) prepareSplitText(el, binding);
	...
	const gsapId = el._gsapId || el.dataset.gsapId;
	if (binding.modifiers.whenVisible) timelineOptions.scrollTrigger = {
		trigger: el,
		id: gsapId,
	...
	if (binding.modifiers.parallax) timelineOptions.scrollTrigger = {
		trigger: el,
		id: gsapId,
		start: `top bottom`,
		end: `bottom top`,
	...
	if (!once && binding.modifiers.parallax) timelineOptions.scrollTrigger.toggleActions = "restart none none reverse";
	...
	if (binding.modifiers.parallax) {
		...
		timeline.fromTo(el, { yPercent: +`${10 * +(parallaxFactor || 5) * direction}` }, {
			yPercent: +`${10 * +(parallaxFactor || 5) * direction * -1}`,
			ease: "linear"
		});
	}
	const delayKey = Object.keys(binding.modifiers).find((modifier) => modifier.includes("delay"));
	if (delayKey) {
		const milliseconds = delayKey.split("-")?.[1] || 500;
		timeline.to("body", { duration: +milliseconds / 1e3 });
	}
	let stagger = false;
	if (binding.modifiers.stagger) stagger = binding.value?.stagger ?? binding.value?.[1]?.stagger ?? "0.2";
	else if (binding.modifiers.splitText) {
		if (binding.value?.stagger !== false && binding.value?.stagger !== 0) stagger = binding.value?.stagger ?? .1;
	}
	let animationTarget = el;
	if (binding.modifiers.splitText && el._splitText) {
		if (binding.modifiers.lines) animationTarget = el._splitText.lines;
		else if (binding.modifiers.words) animationTarget = el._splitText.words;
		else animationTarget = el._splitText.chars;
	} else if (binding.modifiers.stagger) animationTarget = el.children;
	...
	const animationType = Object.keys(binding.modifiers).find((modifier) => [
		"to",
		"from",
		"set",
		"fromTo",
	...
	if (animationType == "from") {
		const fromProps = {
			...
			duration: binding.value.duration || .5
		};
	...
	if (binding.modifiers.animateText) {
		...
		const speed = speeds[Object.keys(binding.modifiers).find((modifier) => Object.keys(speeds).includes(modifier)) || ""] || 2;
		timeline.to(el, { text: {
			value,
			speed
		} });
	...
	if (getValueFromModifier(binding, "onState")) {
		...
		observer = new MutationObserver((mutationRecords) => {
			if (!mutationRecords.filter((record) => record.attributeName == `data-${dataKey}`)?.[0]) return;
...
function addMagneticEffect(el, binding) {
	...
	const handleMouseMove = (e) => {
		if (el) {
			...
			const strengthFactor = Object.entries(strengthModifiers).find((entry) => binding.modifiers[entry[0]])?.[1] || 1;
			const distance = Math.sqrt(distanceX ** 2 + distanceY ** 2);
			const centerDistance = Math.sqrt(deltaX ** 2 + deltaY ** 2);
			const magneticDistanceX = width / 3;
			const magneticDistanceY = height / 3;
			const attractionStrength = .45 * strengthFactor;
	...
	intersectionObserver = new IntersectionObserver((entries) => {
		entries.forEach((entry) => {
			if (entry.isIntersecting) (void 0).addEventListener("mousemove", handleMouseMove);
			else (void 0).removeEventListener("mousemove", handleMouseMove);
		});
	});
...
function loadPreset(binding, configOptions) {
	const applyPreset = (preset, binding2) => {
		...
		if (preset.value) {
			if (binding2.modifiers.fromTo) binding2.value = [preset.value[0], {
				...preset.value[1],
				...binding2.value
			}];
			else binding2.value = {
	...
	if (binding.modifiers.preset && !!configOptions?.presets?.length) {
		const preset = configOptions?.presets.find((preset2) => preset2.name == binding.value);
		if (preset) applyPreset(preset, binding);
	}
	if (binding.modifiers.entrance) {
		const preset = entrancePresets.filter((preset2) => Object.keys(binding.modifiers).includes(preset2.name))?.[0];
		if (preset) applyPreset(preset, binding);
	}
	return binding;
}
...
var virtual_nuxt_node_modules_2F_cache_2Fnuxt_2F_nuxt_2Flayouts_default = { default: defineAsyncComponent(() => import('../build/default-CZVPGlFo.mjs').then((m) => m.default || m)) };
//#endregion
...
var _sfc_main$1 = {
	...
	setup(__props) {
		...
		const stack = void 0;
		const _Error404 = defineAsyncComponent(() => import('../build/error-404-DcRaJjlS.mjs'));
		const _Error = defineAsyncComponent(() => import('../build/error-500-DHIVWgG5.mjs'));
...
export { ClientOnly as C, NuxtLink as N, _plugin_vue_export_helper_default as _, useRuntimeConfig as a, useHead$1 as b, components_default as c, useRoute as d, _sfc_main$13 as e, entry as f, useSeoMeta$1 as u };
